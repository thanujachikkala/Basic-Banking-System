...
function check_name($fromuser,$touser,$amount){
    ...
    try {
        ...
        $sql = 'SELECT amount FROM customers WHERE Name=:fromuser';
        $stmt = $db->prepare($sql);
        $stmt->execute(array(":fromuser" => $fromuser));
        ...
        $st3 = $db->prepare(
            "INSERT INTO `transaction` ( `Fromuser`, `Touser`, `Amount`) VALUES ( :fromuser, :touser, :amount)"
        );
        
        //Assigning values to placeholder
        $st1->bindValue(':amt', $amount, PDO::PARAM_INT);
        $st1->bindValue(':fro', $fromuser, PDO::PARAM_STR);
        $st2->bindValue(':amt', $amount, PDO::PARAM_INT);
        $st2->bindValue(':to', $touser, PDO::PARAM_STR);
        $st3->bindValue(':amount', $amount, PDO::PARAM_INT);
        $st3->bindValue(':touser', $touser, PDO::PARAM_STR);
        $st3->bindValue(':fromuser', $fromuser, PDO::PARAM_STR);
