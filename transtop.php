<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <!-- external css bootstrap  -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KyZXEAg3QhqLMpG8r+8fhAXLRk2vvoC2f3B09zVXn8CA5QIVfZOJ3BCsw2P0p/We" crossorigin="anonymous">
    
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Top Transactions</title>
</head>
<body>

    <!-- header with connection and sql code -->
    <?php require_once './include/header.php'; 
        require 'connection.php';
        $toptrans='';?>
    <!-- transfer form starts -->
    <div class="container my-4">
      <h2>Top Transactions</h2>
    </div>
    <div class="container my-3">
  <form action="<?php $_PHP_SELF ?>" method="post" >
    <div class="mb-3">
    <label for="toptrans" class="form-label">Enter number of Transactions</label>
    <input type="number" class="form-control" id="toptrans" name="toptrans" ?>
  </div>
  <button type="submit" class="btn btn-primary" id="submit" >Submit</button>
</form>
</div>


<?php
 $toptrans=$_POST["toptrans"];
 echo $toptrans;
 $sql = "SELECT * FROM `transaction` WHERE Tran_ID <= $toptrans";

 $result = mysqli_query($conn,$sql);
 $num_of_entry = mysqli_num_rows($result); 
  ?>

  <!-- main container -->
  <div class="container my-5">
      <h2>Transaction details</h2>
      <div class="container my-3">
          <!-- table starts -->
            <table class="table table-hover">
                <thead>
                  <tr>
                    <th scope="col">Transaction ID</th>
                    <th scope="col">Fromuser</th>
                    <th scope="col">To</th>
                    <th scope="col">Amount</th>
                    <th scope="col">DateTime</th>
                  </tr>
                </thead>
                 <!-- fetching data from table -->
                <?php 
                  if($num_of_entry>0){
                    while($row = mysqli_fetch_assoc($result)){
                        echo "<tr>
                        <th scope='row'>".$row['Tran_ID']."</th>
                        <td>".$row['Fromuser']."</td>
                        <td>".$row['Touser']."</td>
                        <td>".$row['Amount']."</td>
                        <td>".$row['DateTime']."</td>
                        </tr>";

                    }
                  }else{
                    echo "<script>alert('No data found in table!');</script>";
                  }
                  ?>
                <tbody>
                </tbody>
              </table>
              <!-- table ends -->
              </div>
      </div>
  </div>

     <!-- footer  -->
  <?php require_once './include/footer.php' ?>  
</body>
        <!-- bootstrap js script -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-U1DAWAznBHeqEIlVSCgzq+c9gqGAJn5c/t99JyeKa9xxaYpSvHU5awsuZVVFIhvj" crossorigin="anonymous"></script>
<script src="./js/main.js"></script>
</html>